...
def writeBSDPPlist(plistPath, offers):
    ...
    if os.path.exists(plistPath):
        ...
        try:
            # Update to current values
            dhcpServer = pldata["dhcp"]["dhcpserverip"]
            ip_address = pldata["dhcp"]["ipaddress"]
            # print("Found existing DHCP Settings, Updating Plist...")
        except:
    ...
    for N in range(0, len(offers)):
        ...
        for anNBI in offers[N].vendorOptionResults["nbi_list"]:
            ...
            serverDict["nbis"].append(
                dict(name=offerName, id=str(offerID), default=defaultNBI)
            )
...
class DHCPDiscover:
    ...
    def buildPacket(self, packType):
        ...
        if packType == "dhcp":
            ...
            packet += (
                b"\x39\x02\x05\xdc"  # Option 57, length 2, max dhcp message sizeL 1500
            )
            # Short APLBSDPC
...
class BSDPOffer:
    ...
    def printBSDPOffer(self):
        ...
        if self.vendorOptionResults["nbi_list"][0]:
            if (
                self.vendorOptionResults["default_nbi"]
                == self.vendorOptionResults["nbi_list"][0][0]
            ):
                print(
                    "{0:10s}  (ID: {2:4d}) {1} (Default)".format(
                        " ",
                        self.vendorOptionResults["nbi_list"][0][1],
                        self.vendorOptionResults["nbi_list"][0][0],
                    )
                )
            else:
                print(
                    "{0:10s}  (ID: {2:4d}) {1:15s} ".format(
                        " ",
                        self.vendorOptionResults["nbi_list"][0][1],
                        self.vendorOptionResults["nbi_list"][0][0],
                    )
        ...
        if len(self.vendorOptionResults["nbi_list"]) > 1:
            for i in range(1, len(self.vendorOptionResults["nbi_list"])):
                print(
                    "{0:10s}  (ID: {2:4d}) {1:15s} ".format(
                        " ",
                        self.vendorOptionResults["nbi_list"][i][1],
                        self.vendorOptionResults["nbi_list"][i][0],
                    )
                )
    ...
    def printBSDPDetailOffer(self):
        ...
        print(
            "{0:20s} : {1:3d}".format(
                "Default Image ID ", self.vendorOptionResults["default_nbi"]
            )
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Check, Print, and Save DHCP & BSDP Information."
    )
    parser.add_argument(
        "-d",
        "--dhcp",
        action="store_true",
        dest="choiceDHCP",
        help="Send DHCP Broadcast and process results.",
    )
    parser.add_argument(
        "-b",
        "--bsdp",
        action="store_true",
        dest="choiceBSDP",
        help="Send BSDP Inform and process results.",
    )
    parser.add_argument(
        "-p",
        "-plist",
        action="store",
        default="/Library/Preferences/edu.psu.sdt.plist",
        dest="plistPath",
